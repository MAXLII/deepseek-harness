 ...
 * until a project Workspace is selected. A fresh world has no Workspace, so
 * startup opens projectless chat; scenarios that exercise repository tools
 * connect one first. The picker now offers both Projectless chat and Add
 * workspace, so the helper chooses the latter explicitly. The directory is staged here

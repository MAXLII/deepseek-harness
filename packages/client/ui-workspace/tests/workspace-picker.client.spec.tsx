...
function mount(
  ...
  return {
    view, onPick, onProjectless, onClose, createWorkspace, probe, occupancy,
...
describe('WorkspacePicker', () => {
  it('offers projectless chat without a workspace and forwards the explicit mode', () => {
    const b = mount([])
    fireEvent.click(screen.getByRole('menuitem', { name: '纯聊天（不使用工作区）' }))
    expect(b.onProjectless).toHaveBeenCalledOnce()
  ...
  it('keeps the chooser visible when projectless chat and add-workspace are the two available paths', () => {
    const b = mount([])
    expect(screen.getByRole('menu')).toBeTruthy()
    expect(screen.getByRole('menuitem', { name: '添加工作区…' })).toBeTruthy()
    expect(screen.getByRole('menuitem', { name: '纯聊天（不使用工作区）' })).toBeTruthy()
  ...
  it('waits to show its menu until an optional anchor is available', () => {
    ...
    render(
      <WorkspacePicker
        ...
        onPick={vi.fn()} onProjectless={vi.fn()} projectless={false} onClose={vi.fn()} createWorkspace={vi.fn()}
  ...
  it('still offers projectless chat when nothing is listed and no directory picker is composed', () => {
